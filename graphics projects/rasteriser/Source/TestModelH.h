...
public:
	...
	Triangle( glm::vec4 v0, glm::vec4 v1, glm::vec4 v2, glm::vec3 color, bool hypertexture )
		: v0(v0), v1(v1), v2(v2), color(color), hypertexture(hypertexture)
	{
		ComputeNormal();
	}
...
// struct to store the depth of each pixel for creation of a depth map
struct Pixel  {
	int x;
	int y;
	float zinv;
...
// struct to store vertices and cool facts about them
struct Vertex {
	glm::vec4 position;
};

//struct to store slices used for composing the hypertexture
struct Slice {
	Pixel topLeft;
  Pixel topRight;
  Pixel bottomLeft;
...
//struct to store values required for hypertexture layer composition
struct TransBufferElement {
  float transparency;
  vec3 colour;
  float density; //used in normal calculation
  glm::vec4 pos3d;//used in normal calculation
};
...
void LoadTestModel( std::vector<Triangle>& triangles, std::vector<Triangle>& hypertextures, glm::vec4& bottomLeft, glm::vec4& bottomRight, glm::vec4& topLeft, glm::vec4& bottomLeftBack )
{
	...
	E = vec4(290,300,114,1);
	F = vec4(130,300, 65,1);
	G = vec4(240,300,272,1);
	H = vec4( 82,300,225,1);
